...
let exportedMethods = {
    ...
    getEvent(id)  {
        ...
        return peopleData.getPeoplebyList(event.attendees).then(list => {
            return locationData.getLocation(event.location).then( location => {
                let re = {
                    id: event.id,
                    attendees: list,
                    title: event.title,
    ...
    getEventsForLocation(locationId) {
        ...
        console.log('getEventsForAttendee is called');
